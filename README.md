...
- `src/content.mjs`：隔离环境中的消息桥接、快捷键。不自动开启托管。
- `src/background-core.mjs`：设置、会话、权限、Jev 协议和错误处理。会话计数存于 session storage，后台休眠/唤醒不重置预算；以标签页、文档和会话标识拒绝过期响应。
