...
设置页底部新增「决策日志」：扩展在本机记录每次模型问答（各决策组的说明、候选、选择、置信度、概率）、执行结果（受理 / 等待 / 未执行及原因）、请求失败、托管开始结束和对局结果，附带当时的资金、部队、电力等数字摘要；不记录密钥、会话令牌和原始单位列表。最多约 2000 条或 4 MB，超出时丢弃最旧的。面板直接显示汇总：决策次数、平均响应、受理 / 等待 / 未执行、已受理的生产、各决策组的等待率与常选项。「导出 JSON」把汇总和全部记录下载为 `jev-log-<时间>.json`，「清空日志」删除本机记录。

离线分析：`node tools/analyze-log.mjs <导出的 json>` 打印同样的汇总并列出全部选择等待的比例。本地 Laya 服务加 `--log <文件>` 会把每次请求另存为 JSON Lines（候选、选择、概率、耗时、状态长度），同一脚本也能分析：`node tools/analyze-log.mjs <该文件>`。
