...
const mock=`
let settings={apiBase:'https://api.typesafe.ai/v1',model:'jev-latest',hotkey:'Alt+Shift+J',autoCamera:true,showOverlay:true,maxDecisions:2000,hasKey:true,language:localStorage.previewLanguage||'zh-CN'};
const now=Date.now();
...
window.chrome={permissions:{request:async()=>true},tabs:{query:async()=>[{id:1}]},runtime:{sendMessage:async m=>{
 ...
 if(m.type==='TEST_CONNECTION')return {ok:true,value:{latencyMs:123}};
 if(m.type==='GET_STATUS'){if(status.observation)status.observation.at=Date.now();return {ok:true,value:status};}
 if(m.type==='SAVE_SETTINGS'){const {apiKey,...rest}=m.settings;settings={...rest,hasKey:!!apiKey||settings.hasKey};return {ok:true,value:settings};}
 if(m.type==='CLEAR_KEY'){settings.hasKey=false;status.running=false;return {ok:true,value:{}};}
