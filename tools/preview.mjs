...
const mock=`
let settings={provider:localStorage.previewProvider||'jev',providerName:(localStorage.previewProvider||'jev')==='local'?'Laya':'Jev',apiBase:'https://api.typesafe.ai/v1',model:'jev-latest',localBase:'http://127.0.0.1:8742/v1',localModel:'laya',hasLocalKey:false,hotkey:'Alt+Shift+J',autoCamera:true,showOverlay:true,maxDecisions:2000,hasKey:!new URL(location.href).searchParams.has('nokey'),language:localStorage.previewLanguage||'zh-CN'};
const now=Date.now();
