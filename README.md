...
## 0.4.3：后台标签页不再停摆、舰船自卫、对局战绩

- **决策循环改由游戏拍子唤醒**。此前决策和微操只靠页面定时器驱动，Chrome 对后台标签页会把定时器压到每分钟一次，而游戏模拟照常推进，于是切走一会儿就会出现「每分钟只问一次、部队原地不动、钱越攒越多」。现在玩家 API 的 `onTick` 每拍检查一次，定时器迟到就立即补跑，标签页在后台也保持正常节奏；停止托管时注销回调。
- **舰船自卫**：海军单位射程内出现敌人时自动开火，即使正在执行模型下达的打击或侦察命令；不参与陆战任务和工厂通道清理。
- **战绩**：弹窗新增「战绩」标签。每次托管结束（手动、胜负、对局结束、页面刷新）自动记录一场：时长、游戏内时长、模型来源与模型名、请求 / 回答 / 失败次数、平均响应、受理 / 等待动作数、部队峰值、资金起止与峰值、资金与决策曲线、已受理生产、各决策组等待率、本局目标与结束原因。最多保留 30 场，可逐场导出 JSON 或清空。
- 决策日志的状态摘要新增出击就绪状态与原因。

## 0.4.2：按实际情况决定何时进攻，支持战役目标
